...
/*
 * ext4_ext_reload_paths -	Reload the paths in a cursor starting
 * 				from the level having invalid pointer
 *
 * @cur:	Cursor to an extent tree
 * @depth:	The level to start the reload at
 * @right:	Try to load the rightmost children
 *
 * Return 0 on success, EIO on currupted block, or return values of fs_bread().
 */
int
ext4_ext_reload_paths(struct ext4_ext_cursor *cur, int depth, bool right)
{
	int ret = 0;
	int i;
	struct ext4_extent_header *hdr;
	...
	for (i = depth - 1; i >= 0; i--) {
		...
		if (cur->c_paths[i].p_hdr &&
			...
			ext4_idx_block(idx)) {
			nritems = ext4_ext_header_entries(cur->c_paths[i].p_hdr);
			ext4_assert(nritems);
			if (cur->c_paths[i].p_ptr == -1 ||
			    cur->c_paths[i].p_ptr >= nritems) {
				if (right)
					cur->c_paths[i].p_ptr = nritems - 1;
				else
...
ext4_ext_decrement(struct ext4_ext_cursor *cur, bool *noprevp)
{
	...
	cur->c_paths[depth].p_ptr--;
	ret = ext4_ext_reload_paths(cur, depth, true);
	if (ret)
		goto out;
...
ext4_ext_increment(struct ext4_ext_cursor *cur, bool *nonextp)
{
	...
	cur->c_paths[depth].p_ptr++;
	ret = ext4_ext_reload_paths(cur, depth, false);
	if (ret)
		goto out;
...
/*
 * ext4_ext_delete_leaf - Remove extents in a leaf starting
 * 			  from the current extent and having
 * 			  key less than or equal to @tolblk.
 *
 * @cur:	Cursor to an extent tree
 * @tolblk:	The logical block
 * @stopp:	A place to return whether the caller should
 * 		stop deletion. Will be set to true if an
 * 		extent having key greater than @tolblk is
 * 		met.
 *
...
/*
 * ext4_ext_delete_node - Remove the current index at specified
 * 			  level.
 *
 * @cur:	Cursor to an extent tree
 * @depth:	The level where deletion takes place at
 *
 * Return 0 if there is no error, or return values of blocks
 * freeing routine.
 */
 ...
 * 				and specify @fromlblk as the parameter to
 * 				the lookup routine before calling into this
 * 				routine.
 *
 * @cur:	Cursor to an extent tree
 *
		      ...
		      ext4_lblk_t tolblk)
{
	...
	if (!depth) {
		...
		if (!ext4_ext_tree_empty(cur)) {
			/*
			 * The tree cursor's pointer is now beyond the
			 * The last valid extent. Reload the cursor's
			 * path so that it points to the last valid
			 * extent.
			 */

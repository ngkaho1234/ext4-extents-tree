...
#define ext4_mark_inode_dirty(handle, n) 0
...
static inline int ext4_allocate_single_block(struct inode *inode,
					     ...
					     ext4_fsblk_t *blockp)
{
	*blockp = simple_balloc(inode->i_sb,
				inode->i_sb->s_blocksize);

	return 0;
}

static ext4_fsblk_t ext4_new_meta_blocks(handle_t *handle, struct inode *inode,
			ext4_fsblk_t goal,
			unsigned int flags,
			unsigned long *count, int *errp)
{
	...
	*errp = ext4_allocate_single_block(inode, goal, &block);
	return block;
}

static void ext4_free_blocks(handle_t *handle, struct inode *inode, void *fake,
			    ext4_fsblk_t block, int count, int flags)
{
}

#define ext_debug(str...)                                           \
	do {                                                                   \
		break;                                                         \
...
#define EXT4_ERROR_INODE(inode, str...) printk(str)
				 ...
				 int logical, ext4_fsblk_t ptr)
{
	...
	if (ix > EXT_MAX_INDEX(curp->p_hdr)) {
	...
	if (ix > EXT_LAST_INDEX(curp->p_hdr)) {
				...
				ext4_lblk_t *logical, ext4_fsblk_t *phys)
{
	...
	*phys = 0;
	...
	if (*logical < le32_to_cpu(ex->ee_block)) {
		if (EXT_FIRST_EXTENT(path[depth].p_hdr) != ex) {
			EXT4_ERROR_INODE(inode,
		...
		while (--depth >= 0) {
			ix = path[depth].p_idx;
			if (ix != EXT_FIRST_INDEX(path[depth].p_hdr)) {
				EXT4_ERROR_INODE(inode,
				...
				int gb_flags)
{
	...
	if (ext4_ext_get_actual_len(newext) == 0) {
...
/*
 * The above routines are responsible for extents tree metadata manipulation.
					   ...
					   int flags)
{
	...
	if (EXT4_EXT_MAY_ZEROOUT & split_flag)
		/* max_zeroout default value is 32KB. */
		max_zeroout = 32 >>
			      ...
			      ext4_lblk_t from, ext4_lblk_t to)
{
	...
	if (from >= le32_to_cpu(ex->ee_block)
		...
		printk(KERN_ERR "strange request: removal(2) "
			   "%u-%u from %u:%u\n",
			  ...
			  ext4_lblk_t end)
{
	...
	handle = ext4_journal_start(inode, 0, depth + 1);
	if (IS_ERR(handle))
			...
			int create, int extend_disksize)
{
	...
	newblock = ext4_new_meta_blocks(handle, inode, goal, 0,
					&allocated, &err);
	if (!newblock)
		goto out2;

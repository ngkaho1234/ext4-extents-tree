...
struct buffer_head
{
	...
	pthread_mutex_t b_writeback_lock;
};
...
static inline int trylock_buffer_wb(struct buffer_head *bh)
{
	return !pthread_mutex_trylock(&bh->b_writeback_lock);
}

static inline void lock_buffer_wb(struct buffer_head *bh)
{
	pthread_mutex_lock(&bh->b_writeback_lock);
}

static inline void unlock_buffer_wb(struct buffer_head *bh)
{
	pthread_mutex_unlock(&bh->b_writeback_lock);
}
...
struct buffer_head *buffer_alloc(struct block_device *, uint64_t, int);
...
int submit_bh(int is_write, struct buffer_head *);
int bh_submit_read(struct buffer_head *bh);
uint64_t simple_balloc(struct super_block *, int);
struct block_device *bdev_alloc(int);
void bdev_free(struct block_device *);
